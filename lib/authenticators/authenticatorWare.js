...
{
    var collated = new RequestModel(results[0].value.request);
    for(const result of results)
    {
        collated.privileges.push(result.value.privileges);
        collated.errors.push(result.value.errors);
        collated.errorCodes.push(result.value.errorCodes);
    };
